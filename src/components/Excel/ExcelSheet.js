...
//   const companyContext = useContext(CompanyContext);
  ...
  const handleExport = () => {
    ...
    //   {
    //     A:
    //       "For the Period: " + format(new Date(dateFrom), "yyyy-MM-dd").toString() +" "+"to: "+ format(new Date(dateTo), "yyyy-MM-dd").toString(),
    //   }
    // ];
    let table1 = data[0]
  ...
  return (
    <button
      className="flex flex-row bg-none text-btn2 border px-2 rounded-lg border-btn2 gap-2 items-center hover:bg-btn2 hover:text-white font-semibold"

      onClick={
        createDownLoadData
      }
    >
      <SiMicrosoftexcel className='0' /><p className=''> Excel</p>
    </button>
  )
}
